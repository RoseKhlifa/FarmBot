...
export function getIllustrated<T = any>(refresh = false, illustratedType = 1, config?: ApiRequestConfig) {
...
export function buyIllustrated<T = any>(payload: Record<string, unknown> = {}, config?: ApiRequestConfig) {
  return client.post<ApiEnvelope<T>>('/api/illustrated/buy', payload, config)
}

export function buyAllIllustrated<T = any>(illustratedType = 1, config?: ApiRequestConfig) {
