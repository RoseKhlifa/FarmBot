...
async function fetchStatus() {
  ...
  errorText.value = ''
  try {
    const res = await friendApi.getDogGifts({ timeout: 20000 })
    if (res.data.ok) {
...
async function claim() {
  ...
  errorText.value = ''
  try {
    const res = await friendApi.claimDogGifts({ timeout: 30000 })
    if (res.data.ok) {
...
<template>
  <div class="space-y-4">
    ...
    <div class="rounded-lg bg-blue-50 p-3 text-sm text-blue-800 dark:bg-blue-900/20 sm:p-4 dark:text-blue-200">
      ...
      <div class="mt-2 flex items-center gap-1.5 rounded-lg bg-amber-100 px-2.5 py-1.5 text-xs text-amber-700 font-medium dark:bg-amber-900/30 dark:text-amber-300">
